...
export default function ResetPassword() {
  ...
  let validationSchema = Yup.object({
    ...
    email:Yup.string().email("enter valid email").required("email is required"),
     newPassword:Yup.string().required("new password is required").matches(/^[A-Z][a-z0-9]{5,10}$/ , "password must start with uppercase letter ")
...
<div className='bg-white text-main  fs-1'>
 ...
 <div className='bg-white text-main fs-1'>
  ...
  <h3 className='fw-bold my-2 title position-relative'>Reset Password</h3>
...
<div className='my-2'>
  ...
  <div className='my-2'>
        <label htmlFor="name" className='fw-bolder'>New Password</label>
            <div className=" d-flex justify-content-between align-items-center border border-3 rounded-3 p-2">
              <i class="fa-solid fa-lock"></i>
      <input placeholder='Enter Your new password' onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.newPassword} type="password" name = "newPassword" id='newPassword' className='form-control fw-bold' />
      </div>
       {formik.errors.newPassword && formik.touched.newPassword?<div className='alert'>{formik.errors.newPassword}</div>:""}
  </div>
